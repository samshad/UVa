    ...
    while(si(n) != EOF){
        ...
        FOR(i, 0, n){
            FOR(l, 0, n){
                if(z[l] + z[i] == m){
                    if(abs(z[l] - z[i]) < s){
                        a = z[i], b = z[l], s = abs(z[l] - z[i]);
                    }
                }
            }
        }

        if(a > b) pf("Peter should buy books whose prices are %d and %d.\n\n", b, a);

        else pf("Peter should buy books whose prices are %d and %d.\n\n", a, b);

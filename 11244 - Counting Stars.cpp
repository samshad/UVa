...
int main(){
    ...
    while(sii(r,c) && r && c){
        ...
        FOR(l, 1, r + 1){
            rep(i,c){
                if(a[l][i] == '*'){
                    if(i > 0 && (a[l-1][i] != '*' && a[l-1][i-1] != '*' && a[l-1][i+1] != '*' && a[l][i+1] != '*' && a[l][i-1] != '*' && a[l+1][i+1] != '*' && a[l+1][i-1] != '*' && a[l+1][i] != '*'))
                        s++;

                    else if(i == 0 && a[l-1][i] != '*' && a[l-1][i+1] != '*' && a[l][i+1] != '*' && a[l+1][i+1] != '*' && a[l+1][i] != '*')
                        s++;
                }
            }
        }

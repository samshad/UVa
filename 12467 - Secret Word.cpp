...
/*int bs(int key) {
    ...
    return lo; //returns the lowest index where arr[lo] >= key;
}*/
...
bool chq(int x){
    if(!x) return 1;
    LL pre = h.getHash(0,x-1);
    //LL suf = h.getHash(sz(a)-x,sz(a)-1);
    int i = 0, j = i + x - 1;
    while(j < sz(a)){
        LL a = g.revHash(i,j);
        if(a == pre) return 1;

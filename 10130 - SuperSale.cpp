...
int n, p[siz], w[siz], dp[1010][110], bag;

int go(int pos, int www){
    if(pos >= n) return 0;

    if(dp[pos][www] != -1) return dp[pos][www];

    int ans1 = 0, ans2 = 0;

    if(www + w[pos] <= bag)
        ans1 = p[pos] + go(pos + 1, www + w[pos]);

    ans2 = go(pos + 1, www);

    dp[pos][www] = max(ans1, ans2);
